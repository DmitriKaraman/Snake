...
def get_safe(my_body,my_length,snakes, food, my_health):
    my_head = my_body[0]
    ...
    for move, isSafe in is_move_safe.items():
        if isSafe:              
            if possible_moves[move] in food:
                if my_health < 25:
                    print("FOOD: YES", move.upper(), possible_moves[move], food)
                    is_move_safe[move] = True
                    
                else:
                    print("FOOD: NO", move.upper(), possible_moves[move], food)
                    is_move_safe[move] = False
                    
                    if is_move_safe["left"] == False and is_move_safe["right"] == False and is_move_safe["down"] == False and is_move_safe["up"] == False:
                        print("FOOD: SURE", move.upper(), possible_moves[move], food)
                        is_move_safe[move] = True    
...
def move(game_state: typing.Dict) -> typing.Dict:
    ...
    is_move_safe = get_safe(my_body,my_length,garbage_snakes, food, my_health)
    print(is_move_safe)
